...
class UserController {
    async create({ request, response}){
        ...
        if(request.input('email') && request.input('password') && request.input('email')){
            ...
            if(request.file('pic')){
                ...
                await pic.move(Helpers.tmpPath('profiles'), {name: image_name,overwrite: true})        

                if(!pic.moved()){

                    resopnse.status(500).json({massage: "Error saving image",error: profilePic.error()})}
                ...
                await user.save();
                response.status(200).json({massage: "user created succefully",data: user})
    ...
    async store({request, response, auth}){
        try{
            ...
            try{
                ...
                return response.status(202).json({massage: "Loged in",user: user,token: token})
    ...
    async logout({response, auth}){
        try{
            ...
            try{
                ...
                response.status(200).json({massage: "loged out"})

            }catch(e){

                response.status(500).json({massage: "Error  loging out",error: e})
    ...
    async edit({request, response, auth}){
        ...
        try{
                ...
                await user.save()
                return response.status(200).json({massage: "Profile edited succefully",user: user})

        }catch(e){

                return response.status(500).json({massage: "Eror editing profile",error: e})
    ...
    async delete(){
        ...
        try{
            ...
            await user.delete()
            return response.status(200).json({massage: "User deleted successfully"})

        }catch(e){

            return response.status(500).json({massage: "Eror deleting profile",error: e})
    ...
    async getUser({params, response}){
            try{
                ...
                return response.status(200).json({massage: "User loaded succefully",user: user,posts: posts})

            }catch(e){

                return response.status(500).json({massage: "Error loading user",error: e})
