...
class ReadingListController {
    async load({request, response, auth}){ //Loading saved posts by user
        try{
            ...
            return response.status(401).json({massage: "User is not authenticated"})
    ...
    async save({request, response, auth}){ //Saveing a post in the list
        try{
            ...
            try{
                ...
                return response.status(404).json({massage: "Post doesn't exist"})
            ...
            if(!posts_ids.includes(post)){
                ...
                try{
                    ...
                    await user.save()
                    return response.status(200).json({massag: "Post saved succesfully"})

                }catch(e){

                    return response.status(500).json({massage: "Error saving post",error: e})
                ...
                try{
                    ...
                    await user.save()
                    return response.status(200).json({massag: "Post unsaved succesfully"})

                }catch(e){

                    return response.status(500).json({massage: "Error unsaving post",error: e})
